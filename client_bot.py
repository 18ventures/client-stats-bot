...
def handle_status(chat_id: str):
    try:
        ...
        if position:
            ...
            unreal   = float(position["unRealizedProfit"])
            roi_pct  = (unreal / (abs(amt) * entry / LEVERAGE)) * 100 if entry else 0
            pos_val  = abs(amt) * entry
            liq      = float(position.get("liquidationPrice", 0))

            pos_text = (
                f"📊 <b>Open Position: {side}</b>\n"
                f"Symbol: {SYMBOL}\n"
                f"Size: ${pos_val:,.2f} (×{LEVERAGE} lev)\n"
                f"Entry: ${entry:,.2f}\n"
                f"Mark:  ${mark:,.2f}\n"
                f"Unrealised: ${unreal:+.2f} ({roi_pct:+.2f}%)\n"
    ...
    while True:
        ...
        for update in updates:
            ...
            if cmd == "start":
                ...
                send_telegram(chat_id, "Unknown command. Try /status, /pnl, /balance or /history.")
